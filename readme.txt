=== Privacy Policy Generator, Terms & Conditions, GDPR, CCPA, Cookie Policy & Disclaimer Templates – WPLP Legal Pages ===
Contributors: wplegalpages
Donate link:  https://wplegalpages.com/
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: privacy policy generator, terms and conditions, legal pages, GDPR, disclaimer
Requires at least: 5.0
Requires PHP: 7.0
Tested up to: 7.0.2
Stable tag: 3.7.2

== DESCRIPTION ==

**Generate a privacy policy, terms and conditions, cookie policy, disclaimer, and 30+ other legal pages for your WordPress site — answer a few questions and publish in minutes. No lawyer, no monthly fee, no coding.**

[WPLP Legal Pages](https://wplegalpages.com/legal-pages/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) runs entirely inside WordPress. You answer a short questionnaire about your site, and the plugin generates complete policies using your business name, contact details, and the data you actually collect — then publishes them as real pages on your own domain. Every word is editable in the standard WordPress editor afterwards.

Trusted by [10,000+] WordPress sites. 30+ ready-made templates. Live in about ten minutes.

[youtube https://www.youtube.com/watch?v=93LIh4ARMN8]
*Watch the 3-minute setup: from install to a published, GDPR-ready privacy policy.*

= What you can publish =

A library of 30+ templates covering the documents most websites, stores, and SaaS products are asked for:

* Privacy Policy (GDPR and CCPA-ready)
* Terms and Conditions / Terms of Use
* Cookie Policy
* Disclaimer
* Affiliate Disclosure (FTC-compliant)
* Refund and Returns Policy
* DMCA Policy
* Children's Privacy Policy (COPPA)
* Earnings Disclaimer, Medical Disclaimer, EULA
* Industry- and region-specific disclosures

Each one is generated from your answers — not handed to you as a blank template with square brackets to fill in. Review, edit, publish.

= How it works =

1. **Install and activate.** No account to create, no API key, no external service.
2. **Answer the questionnaire.** Your business name, contact details, what data you collect, and whether you run ads, affiliates, or a store.
3. **Review and publish.** Pages are generated with your details already filled in. Edit anything, then publish. Footer links can be added automatically.

= What makes WPLP Legal Pages different =

* **Your pages live on your own domain** — generated as standard WordPress pages in your own database, not hosted on a third-party service
* **No per-visitor or pageview limits** — no plan caps, whatever your traffic
* **No recurring fee** — the core plugin is free and premium is a [one-time / annual] license, not a monthly subscription
* **Nothing stops working if you stop paying** — your published pages stay published
* **Fully editable in WordPress** — open any generated page in Gutenberg, Elementor, or Divi and rewrite it
* **Nothing leaves your site** — generation happens on your own installation; your policies and your visitors' data stay with you
* **Part of the [WPLP Compliance Platform](https://wplegalpages.com/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages)** — works with WPLP Cookie Consent so your banner and your cookie policy stay in sync

= Features =

**Get a record that visitors actually accepted your terms**

* Require agreement on selected legal pages before a visitor can access content or complete an action
* Show legal notices during account creation, checkout, or form submission
* Attach agreement links to standard WordPress and WooCommerce checkout forms

**Keep under-18 visitors out of age-restricted content**

For sites selling alcohol, vape products, cannabis, tobacco, or firearms accessories, and for 18+ content:

* A configurable age-verification popup that asks visitors to confirm their age before entering
* Your own wording, matched to your product and jurisdiction
* Full control over which pages and posts it appears on, and when it triggers

**Add FTC disclosures without editing 200 posts by hand**

* One-click insertion of affiliate disclosures on selected posts or pages
* Apply to individual content or across multiple pages at once
* Fully editable disclosure text so it matches your site's voice and your network's requirements
* Disclosures land where the FTC expects them — visible, near the affiliate link, on every post that has one

**Keep policies accurate as your business changes**

* Edit and update legal pages from the WordPress dashboard as your site or business model changes
* Display update notices when a policy changes so returning visitors see it
* Automatic footer links keep every page reachable from anywhere on your site

**Pages that look like they belong on your site**

* Edit generated content directly in the WordPress editor
* Add legal page links to your footer automatically
* Control visibility and placement of each link across your site

= Free vs Premium =

The free plugin generates complete, publishable policies — not previews, not watermarked drafts, and no per-visitor limits.

**Free includes:**

* 4 legal templates — Standard Privacy Policy, Terms of Use, DMCA, Standard CCPA
* The guided questionnaire and page generator
* One-click affiliate disclosure insertion
* Automatic footer links and update notices
* Agreement requirement and age verification, in-WordPress
* No per-page or per-visitor limits

**Premium adds:**

* 25+ additional templates — GDPR Privacy Policy, Refund Policy, Earnings Disclaimer, Medical Disclaimer, EULA, Affiliate Disclosure and more
* Advanced age verification — more display and targeting controls for age-gated content
* Announcement banners and custom popups for site-wide notices
* Advanced footer link integration and policy change flagging
* Priority support from the WPLP team

See the [pricing page](https://wplegalpages.com/pricing/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) for more details.

= Privacy and consumer protection frameworks covered =

Templates are written to support disclosures under:

* GDPR and ePrivacy (European Union)
* CCPA and CPRA (California, United States)
* LGPD (Brazil)
* COPPA (United States)
* PIPEDA (Canada)
* Other regional privacy and consumer protection requirements

**Please read this:** WPLP Legal Pages generates documents from templates based on the information you provide. It is not a law firm and does not provide legal advice. If you operate in a regulated industry, process sensitive personal data, or serve a high-risk market, have a qualified attorney review your policies before you rely on them.

= Who uses WPLP Legal Pages =

* **Bloggers and content creators** — the privacy policy, disclaimer, and affiliate disclosures AdSense and affiliate programs ask for
* **Affiliate marketers** — FTC-compliant disclosures across an entire post archive, applied in one pass
* **WooCommerce and eCommerce stores** — refund, returns, and privacy policies alongside checkout, and the terms payment processors ask for during onboarding
* **Agencies** — legal pages done on every client build, without billing hours for it
* **Business websites** — GDPR, CCPA, and consumer-protection disclosures kept current as the business changes
* **SaaS and membership sites** — terms of service, EULA, and age verification for gated content

= Works with WPLP Cookie Consent =

WPLP Legal Pages is a core module of the [WPLP Compliance Platform](https://wplegalpages.com/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages), alongside [WPLP Cookie Consent](https://wordpress.org/plugins/gdpr-cookie-consent). Used together, your cookie banner and your cookie policy stay linked and consistent — when your consent settings change, your policy reflects it. Two plugins that behave like one system instead of two disconnected tools.

= Setup and compatibility =

* Works with popular themes and page builders — Elementor, Divi, Gutenberg
* Compatible with WooCommerce and common form plugins — WPForms, Contact Form 7, Gravity Forms
* WPML-compatible for multilingual legal pages

= Documentation and Support =

* [Overview](https://wplegalpages.com/docs/wplp-docs/overview/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) — what the WPLP Compliance Platform does and how the modules fit together
* [Compliance Setup Wizard](https://wplegalpages.com/docs/wplp-docs/features-walkthrough/compliance-setup-wizard/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) — step-by-step configuration for your site
* [Guides](https://wplegalpages.com/docs/wplp-docs/guides/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) — detailed tutorials for every plugin feature
* [FAQs](https://wplegalpages.com/docs/wplp-docs/troubleshooting-and-support/faqs/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages) — common questions and troubleshooting

For questions about features or compatibility, [contact the WPLP Legal Pages support team](https://wplegalpages.com/contact-us/?utm_source=+wordpress.org&utm_medium=referral+&utm_campaign=wplegalpages).

== Screenshots ==
1. WPLP Legal Pages Policies
2. WPLP Legal Pages Business Details
3. WPLP Legal Pages Policy Template Preview
4. WPLP Legal Pages Getting Started
5. WPLP Legal Pages Policy Settings
6. WPLP Legal Pages Popups
7. WPLP Legal Pages Verify age popup
8. WPLP Legal Pages Recent Policies Preview
9. WPLP Legal Pages Advanced Settings

== Frequently Asked Questions ==

**What legal policies can I generate with the free version?**
Privacy Policy, Terms and Conditions, CCPA Privacy Notice, and DMCA Policy.

**What additional templates does WPLP Legal Pages Pro include?**
25+ premium templates, including GDPR Privacy Policy, Cookie Policy, Refund Policy, Earnings Disclaimer, EULA, Medical Disclaimer, Affiliate Disclosure, and more, plus footer link integration, announcement banners, custom popups, and age verification.

**Does WPLP Legal Pages help with GDPR and CCPA compliance?**
Yes. Pro includes GDPR-compliant Privacy and Cookie Policy templates and a CCPA Privacy Notice to help align with major data protection laws.

**Is WPLP Legal Pages compatible with all WordPress themes?**
Yes. It works with all modern WordPress themes and page builders.

**Can I use WPLP Legal Pages on multiple sites?**
The free version is limited to one site. Pro plans are available based on the number of sites you need.

**Does WPLP Legal Pages support Amazon affiliate or earnings disclaimers?**
Yes. Amazon Affiliate Disclosure and Earnings Disclaimer templates are available.

**Will a templated privacy policy hurt my SEO?**
No. Legal pages are standard boilerplate by design and search engines treat them as such. Duplicate-content concerns apply to your commercial and editorial pages, not to legal notices.

**Will this get my AdSense application approved?**
A missing privacy policy is one of the most common reasons AdSense rejects a site, so publishing one removes that specific blocker. It can't guarantee approval — AdSense also assesses content quality, traffic sources, and site structure.

**What happens to my pages if I deactivate the plugin?**
They stay published. Your legal pages are ordinary WordPress pages stored in your own database. Deactivating removes the generator and its controls, not your content.

**Is any of my data or my visitors' data sent to your servers?**
No. Generation happens on your own installation. Your policies and your visitors' interactions with them stay on your site.

**Will this slow my site down?**
The generator runs in the admin area only. Front-end output is limited to the features you switch on — footer links, consent gates, or the age popup.

**Do I still need a lawyer?**
For most blogs, small stores, and service sites, a complete and accurate generated policy is what's needed and what's expected. If you're in a regulated industry — health, finance, children's services — or you process sensitive data at scale, get a review.

**Can I translate my legal pages?**
Yes. Generated pages are standard WordPress content and work with WPML and similar translation plugins.

**Can I edit the generated text?**
All of it. Pages open in the standard WordPress editor. Most site owners adjust the tone of a paragraph or two and publish.


== Changelog ==

= 3.7.2 =
* **Improvements**
    * Strengthened the codebase with improved security and validation across the plugin.
    * General performance, stability and compatibility improvements.

* **Bug Fixes**
    * Minor bug fixes and improvements.

= 3.7.1 =
* **New**
    * Added a new Coordinated Vulnerability Disclosure Policy template to help organizations establish a responsible process for security researchers to report vulnerabilities.
    * Added a new EU Declaration of Conformity template to help businesses document and communicate compliance with applicable European Union requirements.

= 3.7.0 =
* **Improvements**
    *Improved the dashboard UI to provide a smoother and more intuitive onboarding experience.

= 3.6.9 =
* Minor bug fixes and improvements.

= 3.6.8 =
* Introduced an AI-Powered Compliance Setup Wizard that helps users configure legal pages and cookie compliance settings based on their business type, website purpose, visitor locations, and applicable privacy laws.
* AI now provides personalized compliance recommendations and automatically assists with setting up legal policies and cookie consent requirements for regulations such as GDPR and CCPA.

= 3.6.7 =
* **New:** Added an option in the WPLP SaaS dashboard to directly connect your WordPress website with WP Legal Pages for faster onboarding and centralized compliance management.

= 3.6.6 =
* Minor bugfixes and improvements.

= 3.6.5 =
* Fixed minor UI issues related to Terms and Conditions Legalpage fields and Age Verification popup button.

= 3.6.4 =
* Fixed issue where plan cancellation banner was not displaying correctly.
* Fixed footer font changes not persisting after saving settings. 

= 3.6.3 =
* Fixed infinite loading issue when editing a generated legal page from the plugin after creating it via SaaS.
* Fixed an issue where sidebar widgets were not saving properly.

= 3.6.2 =
* Resolved an issue in the Legal Pages Generation Wizard where user progress was not being saved properly.
* Fixed a bug that caused the wizard state to reset unexpectedly, ensuring a smoother and more reliable page creation experience.

    This update makes the setup flow more stable, so users can generate legal pages without interruptions or lost progress.

= 3.6.1 =
* **New Features & Improvements**
* We’ve rolled out some powerful additions to make compliance easier, faster, and more flexible inside your SaaS dashboard:
    * 🟢 **Announcement Banner**: You can now display custom announcement banners across your website. Perfect for sharing updates, promotions, or important compliance notices without touching your code.
    * 🔞 **Age Verification Popup**: Add an age verification layer to your site in minutes. Ideal for businesses that need to restrict access based on age and stay compliant with regulations.
    * 📄 **Legal Pages as Popup (Agreement Mode)**: Show your legal pages (like Terms & Conditions or Privacy Policy) as a popup and require users to agree before proceeding. This helps you capture explicit user consent seamlessly.
    * 🤝 **Affiliate Disclosure**: Easily generate and display affiliate disclosures directly from the dashboard. Stay transparent with your audience and compliant with advertising guidelines.

= 3.6.0 =
* New: Legal pages can now be generated directly from our SaaS platform. Users can log in to their account at app.wplegalpages.com to create and manage legal pages through the new WPLP Compliance Platform experience.

= 3.5.9 =
* Bug Fixes & Improvements:
    * Fixed an issue where the **upgrade popup was not appearing** when users attempted to create a Pro legal page
    * Resolved **a UI issue in the “Create Popup” flow** that caused layout and design inconsistencies
    * Fixed a bug affecting **legal page creation and editing**, improving reliability and stability

= 3.5.8 =
* Added support for enhanced App Dashboard

= 3.5.7 =
* Minor Bug fixes and improvements.

= 3.5.6 =
* Fixed issues related to a plugin endpoint.

= 3.5.5 =
* Improvement: Integrated updates to support the saas dashboard experience.

= 3.5.4 =
* **Bug Fixes**: We fixed some bugs that were causing minor issues in the plugin. Things should now run more smoothly.
* **UI Update**: We made small design changes to the **Create Popup Page** to make it easier to use and look better.

= 3.5.3 =
* Bug fixes and improvements.

= 3.5.2 =
* Fix - Security related issues fixed.

= 3.5.1 =
* Bug fixes and improvements.

= 3.5.0 =
* Major Update - Revamped Dashboard & Settings
* Completely redesigned dashboard UI for a cleaner, more intuitive experience.
* Restructured menu navigation to make key features easier to find.
* Simplified and reorganized settings pages for faster configuration.
* Improved layout consistency across all modules for better usability.
* Bug fixes and improvements.

= 3.4.4 =
* Bug fixes and improvements.

= 3.4.3 =
* Bug fixes and improvements.

= 3.4.2 =
* Improvement: Improved performance and few bug fixes.

= 3.4.1 =
* Improvement: Accessibility improved in accordance with WCAG 2.2 Level AA standards.
* Improvement: Added option to include DBA (Doing Business As) or trading name in legal policies wherever applicable.

= 3.4.0 =
* Improvement: Minor UI improvements.
* Improvement: Improved performance affecting page load speed.

= 3.3.9 =
* Improvement: Minor improvements.

= 3.3.8 =
* Added Gutenberg Block - You can now easily insert any generated legal policy directly into your WordPress post or page using the new WP Legal Pages block in the block editor.
* Minor bug fixes and improvements.

= 3.3.7 =
* Improvement: All newly generated legal policy pages will now be created using the Gutenberg block editor instead of the classic editor. This offers a more modern editing experience, improved customization, and better compatibility with block-based themes.

= 3.3.6 =
* Update: Updated the Professional Privacy Policy, DMCA, COPPA, General Disclaimer, Earnings Disclaimer, Terms and Conditions Pro, and Cookies Policy templates with new AI-related compliance clauses.


= 3.3.5 =
* Bug Fixes: Resolved minor issues to enhance stability and performance.
* UI Enhancements: Improved the design and user experience of the Age Verification Popup for better accessibility and usability.

= 3.3.4.1 =
* Fix: Minor bug fixes and improvements within the plugin.

= 3.3.4 =
* Improvement: Revamped the menu structure for a more intuitive and seamless user experience.
* Fix: Bug fixes and improvements.

= 3.3.3 =
* Fix: Bug fixes and improvements.

= 3.3.2 =
* Fix: Minor bug fixes and improvements within the plugin.

= 3.3.1 =
* Fix: Minor bug fixes and improvements within the plugin.

= 3.3.0 =
* Feature  - Introduced a Unified Dashboard for managing WP Legal Pages and WP Cookie Consent, providing a seamless interface, improved navigation and enhanced user experience.

= 3.2.8 =
* Fix: Fixed minor vulnerability issues.

= 3.2.7 =
* Fix: Minor bug fixes and improvements within the plugin.

= 3.2.6 =
* Fix: MInor bug fixes and improvements within the plugin.

= 3.2.5 =
* Fix: Minor bug fixes and improvements.

= 3.2.4 =
* Feature: Unified Dashboard: The "WP Legal Pages" menu item now serves as a central hub for managing both WP Legal Pages and WP Cookie Consent, streamlining the user experience.

= 3.2.3 =
* Fix: Fixed minor connection related issue for Pro users.

= 3.2.2 =
* Fix: Minor bug fixes and improvements.

= 3.2.1 =
* Fix: Minor bug fixes and improvements.

= 3.2.0 =
* Fix: Minor bug fixes and improvements.

= 3.1.0 =
* Major Update: We've completely revamped the interface to make it more intuitive, user-friendly, and efficient.

= 3.0.0 =
* Feature: Age Verification - Allows only 18+ aged users to access website content after taking consent.
* Feature: Create Popups - You can choose the legal pages that you want to show as pop ups on your website.
* Feature: Adult Content Site - Enabling this will ask first-time visitors will be forced to confirm that he/she is above his/her country's legal age limit.
* Feature: One-click Affiliate Disclosure Addition - Add affiliate disclosure to the end of a page or post.
* Feature: Show Legal Pages in Search - You can choose to show your legal policies in search results.
* Update: Wordpress 6.5 compatibility.

= 2.10.2 =
* Fix: Minor Bug Fixes.

= 2.10.1 =
* Update: Updated our professional privacy policy template with new clauses addressing key privacy regulations.

= 2.10.0 =
* Feature: Revamped UI: Submenus are now organized into tabs for easier navigation to configurations.

= 2.9.6 =
* Fix: Minor bug fixes and improvements.

= 2.9.5 =
* Fix: Minor Bug Fixes.

= 2.9.4 =
* Update: Wordpress 6.4 compatibility.

= 2.9.3 =
* Fix: Fixed vulnerability and minor bugs.
* Update: Added clause for Google Adsense.

= 2.9.2 =
* Update: Wordpress 6.3 compatibility.

= 2.9.1 =
* Update: Wordpress 6.1 compatibility.

= 2.9.0 =
* Feature: Added Wizard to create a Legal Pages.
* Update: UI Improvement for settings Page.
* Update: Updated UTM paramaters for links.
* Feature: Added Standard Privacy Policy Template.
* Feature: Added Standard CCPA Template.
* Feature: Added Terms of Use Template.
* Update: Added shortcode for published pages in 'All Legal Pages'.

= 2.8.1 =
* Fix: Show legal pages in footer.
* Fix: Template names displayed in editor.

= 2.8.0 =
* Update: New Improved Settings UI.
* Update: Wordpress 5.9 compatibility.
* Update: Help mascot added on all the pages.

= 2.7.4 =
* Update: Updated third-party libraries.

= 2.7.3 =
* Update: Added option to revoke consent for anonymous usage data for the user.
* Fix: Code as per WordPress coding standards.

= 2.7.2 =
* Fix: Security related issues fixed.

= 2.7.1 =
* Fix: Security related issues fixed.

= 2.7.0 =
* Update: Option to add Cookie bar.

= 2.6.0 =
* Update: UI Changes on Settings Page.
* Update: Option to add Legal Pages link in Footer.
* Update: Option to add announcement banner.

= 2.5.3 =
* Fix: Minor plugin fix.

= 2.5.2 =
* Update: Minor UI changes on Create Legal Page.

= 2.5.1 =
* Update: Analytics tracking library.
* Fix: PHP 8 compatibility issues fixed.
* Update: Restructured Getting started page.

= 2.5.0 =
* Update: Added Last updated date in policy template.
* Update: Plugin name change.
* Update: Compatibility with latest WordPress 5.8.

= 2.4.9 =
* Update: Update available pro template list.

= 2.4.8 =
* Feature: Widget to add Legal Pages to the sidebar.
* Feature: Easily add Legal Pages to your theme menu.
* Update: Language translations POT file.

= 2.4.7 =
* Fix: Fixed vulnerability issue for analytics within the plugin.

= 2.4.6 =
* Update: Redesigned Getting started page.
* Update: Notice to track usage analytics for the plugin.

= 2.4.5 =
* Update: Support for WordPress multisite integration. Setup Privacy Policy Pages on WordPress Multisite.

= 2.4.4 =
* Feature: Compatible with WPML. Setup privacy policies on multilingual websites.
* Fix: Compatibility with sub-folder based WordPress instances.

= 2.4.3 =
* Update: Test and minor changes for WordPress 5.4.1 compatibility.
* Update: Improved website performance and theme customizer integration.

= 2.4.2 =
* Update: Improved integration with Pro features.

= 2.4.1 =
* Update: Added Getting Started menu to help easy setup of Privacy or any other terms pages on WordPress.

= 2.4.0 =
* Update: Improved compatibility with the Pro Privacy Generation Wizard.

= 2.3.9 =
* Update: Templates and minor plugin updates.

= 2.3.8 =
* Feature: Added Terms of use templates for French and German Translations.

= 2.3.7 =
Update: Minor plugin updates.

= 2.3.6 =
Update: Updated Terms of use template.

= 2.3.5 =
Feature: Added standard California Consumer Privacy Act (CCPA) and Terms and Conditions policy templates.
Update: Made compatible with WordPress 5.3.2.

= 2.3.4 =
Update: Updated link for show credit.
Update: Made compatible with WordPress 5.3.1.

= 2.3.3 =
Update: Minor plugin updates.

= 2.3.2 =
Fix: Fatal error if Analytics SDK is included in two or more plugins installed on the same instance.

= 2.3.1 =
Feature: Added feedback form on plugin deactivation.

= 2.3.0 =
Update: Updated template HTML structure.
Update: Code updated as per WordPress standards.

= 2.2.9 =
Fix: Pro integration issue.

= 2.2.8 =
Fix: Minification issue.

= 2.2.7 =
Fix: Filter for Legal template count.

= 2.2.6 =
Integration of WP Legal Pages Pro version.

= 2.2.5 =
Fixed issue with filter for Legal Pages rendering

= 2.2.4 =
Improved design of Admin screens.
Compatibility with WordPress 5.2.2

= 2.2.3 =
Updated Admin menu for WP Legal Pages Cookies.
Made compatible with WordPress 5.2.1

= 2.2.2 =
Made compatible with WP Legal Pages Cookie Consent Addon.
Fixed broken links.

= 2.2.1 =
Compatible with WordPress 5.1.1

= 2.2 =
Admin panel UI improvement
Template not updated after activation of the plugin

= 2.1 =
Updated EU cookie template and Privacy policy template to remain compliant with upcoming changes to the EU data protection law

=2.0.6 =
Minor dashboard changes.

= 2.0.5 =
Fixed conflict issues.

= 2.0.4 =
Minor changes in dashboard.
Fixed issues related to shortcode rendering.

= 2.0.3 =
Tested upto WordPress version 4.8.

= 2.0.2 =
Added subscription form.

= 2.0.1 =
Change tags.

= 1.5.9 =
Made Compatible with WordPress 4.7.1
Code Cleanup

= 1.5.8 =
Fix give credit option issue.

= 1.5.6 =
Fix for css/js load to prevent conflict with other plugins.

= 1.5.5 =
Rewrite Plugin.

= 1.5.4 =
Added EU Cookies Policy.

= 1.5.3 =
Added Give Credit option.

= 1.5.2 =
Compatible with PHP 7.0.

= 1.5.1 =
Added list of Pro templates

= 1.5 =
Added new DMCA template

= 1.4 =
Updated for Compatibility with WP 4.5.3. Improved Admin Dashboard for easy creation of policy pages

= 1.3 =
WP Legal Pages is now compatible with WordPress 4.5.

= 1.2 =
Added banner to upgrade WP Legal Pages Pro version on plugin dashboard

= 1.1 =
Enhanced security

= 1.0.0 =
Base version of the plugin.
